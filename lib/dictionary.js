...
Dictionary.areAnagrams = areAnagrams;
...
// sort the first word to get its key.  Check all other words to see if they have
// the same key.  If any of htem do not, they are not all anagrams.
function areAnagrams(words) {
  var sorted = sortWord(words[0]);
  var result = words.slice(1).every(function(word) {
    return sortWord(word) === sorted;
  });
  return result;
}
